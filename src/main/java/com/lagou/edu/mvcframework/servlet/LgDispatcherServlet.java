...
import com.lagou.edu.mvcframework.pojo.HandlerExecutionChain;
...
public class LgDispatcherServlet extends HttpServlet {
    ...
    //handlerExecutionChainMappings
    private List<HandlerExecutionChain> handlerExecutionChainMappings=new ArrayList<>();
    ...
    private void initHandlerMapping() {
        ...
        for(Map.Entry<String,Object> entry: ioc.entrySet()) {
            ...
            for (int i = 0; i < methods.length; i++) {
                ...
                //默认拦截所有请求
                HandlerExecutionChain handlerExecutionChain=new HandlerExecutionChain(handler,this.handlerInterceptorList);
                handlerExecutionChainMappings.add(handlerExecutionChain);
...
//        method.invoke() //
        ...
        HandlerExecutionChain handlerExecutionChain=getHandlerChain(req);
        if(null==handlerExecutionChain){
            resp.getWriter().write("index");
            return;
        }

        Handler handler = getHandler(handlerExecutionChain);
        if(handler == null) {
    ...
    private Handler getHandler(HandlerExecutionChain handlerExecutionChain) {
     ...
     * @return
     */
    private HandlerExecutionChain getHandlerChain(HttpServletRequest req) {
        if(handlerExecutionChainMappings.isEmpty()){
            return null;
        }
        String url = req.getRequestURI();
        for(HandlerExecutionChain executionChain: handlerExecutionChainMappings) {
